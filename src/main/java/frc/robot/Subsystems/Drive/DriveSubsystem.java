...
public class DriveSubsystem extends SubsystemBase {
    ...
    return driveRobotRelative(
        () ->
            ChassisSpeeds.fromFieldRelativeSpeeds(
                desiredFieldSpeeds.get(), driveBase.getHeading()));
  }
            ...
            () -> {
              ...
              double angularVelocity =
                  headingController.calculate(
                      driveBase.getHeading().getRadians(), desiredFieldRotation.get().getRadians());
   ...
   */
  public Command driveAndPointAtTarget(
      Supplier<ChassisSpeeds> fieldRelativeSpeeds, Supplier<Translation2d> target) {
    return driveTopDown(
  ...
  public Command orbitPoint(
      Supplier<Double> orbitDirection, Translation2d orbitCenter, double orbitDistance) {
  ...
  public Command trackPose(Supplier<Pose2d> targetPose) {
    return driveFieldRelative(
            () -> {
              ...
              double xOutput =
                  -xController.calculate(currentRobotPose.getX(), currentTargetPose.getX());
              double clampedXOutput =
                  MathUtil.clamp(
                      xOutput, -DriveBaseTunables.MAX_AUTO_SPEED, DriveBaseTunables.MAX_AUTO_SPEED);

              double yOutput =
                  -yController.calculate(currentRobotPose.getY(), currentTargetPose.getY());
              double clampedYOutput =
                  MathUtil.clamp(
