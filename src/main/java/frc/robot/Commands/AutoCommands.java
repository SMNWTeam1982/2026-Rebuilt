...
public class AutoCommands {
    public static Command setShooterNearestHubRPMSupplier(
    ...
    public static Command shootIntoHub(
        ...
        return Commands.sequence(
            drive.stop(),
            setShooterNearestHubRPMSupplier(drive, shooter).asProxy(), // run the shooter commands as proxies so that the PID can run in background
            Commands.deadline(
                Commands.sequence(
                ...
                drive.driveAndPointAtTarget(() -> new ChassisSpeeds(), () -> ShotCalculation.getNearestHubPosition(drive.getRobotPose().getTranslation()))
            ),
            kicker.setIdle(),
